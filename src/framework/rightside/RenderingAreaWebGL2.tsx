...
export default function RenderingAreaWebGL2(props: IRenderingAreaProps) {
  ...
  if (codes && shaderType === ShaderType.ES30 && needUpdate) {
  ...
  return (
    <canvas
      ...
      height={props.height}
      style={{
        display: shaderType === ShaderType.ES30 ? "" : "none",
      }}
    ></canvas>
  );
}
...
async function init(canvas: HTMLCanvasElement) {
  ...
  const fs = getDefaultCode(ShaderType.ES30);

  glProgram = initShaders(gl, vs, fs);
  initVertexBuffers(gl, glProgram);

  function frame() {
    if (store.getState().shader.currentShaderType === ShaderType.ES30) {
...
let hasCanvas = setInterval(() => {
  let canvas = document.getElementById("webgl2Target") as HTMLCanvasElement;
  if (canvas) {
